...
public class DirtyCompanyJob {
    public static void main(String[] args) throws Exception {
        ...
        spark.sql("insert overwrite table test.test_dirty_company_df_result_copy_new partition(pt='20230628') select /*+ REPARTITION(600) */ name from t1 left semi join t2 on substr(reverse(name),0,1) = substr(reverse(dirty_company),0,1) and substr(reverse(name),0,length(dirty_company)) = reverse(dirty_company)");
