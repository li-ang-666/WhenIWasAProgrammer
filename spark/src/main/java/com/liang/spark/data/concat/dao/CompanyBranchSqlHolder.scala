...
class CompanyBranchSqlHolder {
  def queryTotalBranch(): String = {
    s"""
       ...
       |and t1.pt='${DTUtils.getLastNDateTime(1, "yyyyMMdd")}'
  ...
  def queryTotalCanceledBranch(): String = {
    s"""
       ...
       |where t1.pt='${DTUtils.getLastNDateTime(1, "yyyyMMdd")}' and t2.pt='${DTUtils.getLastNDateTime(1, "yyyyMMdd")}'
       |and t1.company_id<>0
       |and t2.dw_is_del=0
  ...
  def queryMostArea(): String = {
    s"""
       ...
       |where pt = '${DTUtils.getLastNDateTime(1, "yyyyMMdd")}'
       |)
