...
class RestrictConsumptionSqlHolder {
  def queryMostApplicant(isHistory: Boolean): String = {
    s"""
       ...
       |where t1.company_id<>0 and t1.pt='${DateUtils.getOfflinePt(1, "yyyyMMdd")}'
  ...
  def queryMostRelatedRestricted(isHistory: Boolean): String = {
    s"""
       ...
       |where t1.is_history=${if (isHistory) 1 else 0} and t1.pt='${DateUtils.getOfflinePt(1, "yyyyMMdd")}'
