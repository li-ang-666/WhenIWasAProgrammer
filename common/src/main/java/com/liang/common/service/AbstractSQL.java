...
public abstract class AbstractSQL<T> {
    ...
    public T REPLACE_INTO(String tableName) {
        sql().statementType = SQLStatement.StatementType.REPLACE;
        sql().tables.add(tableName);
        return getSelf();
    }

    public T INSERT_IGNORE_INTO(String tableName) {
        sql().statementType = SQLStatement.StatementType.IGNORE;
        sql().tables.add(tableName);
        return getSelf();
    }
    ...
    private static class SQLStatement {
        ...
        private String replaceSQL(SafeAppendable builder) {
            sqlClause(builder, "REPLACE INTO", tables, "", "", "");
            sqlClause(builder, "", columns, "(", ")", ", ");
            for (int i = 0; i < valuesList.size(); i++) {
                sqlClause(builder, i > 0 ? "," : "VALUES", valuesList.get(i), "(", ")", ", ");
            }
        ...
        private String ignoreSQL(SafeAppendable builder) {
        ...
        public String sql(Appendable a) {
            ...
            switch (statementType) {
                ...
                case REPLACE:
                    answer = replaceSQL(builder);
                    break;

                case IGNORE:
                    answer = ignoreSQL(builder);
                    break;
